...
2. **A → B: invite code.** A (in that mesh's **Configs ▸ Invite a member** §6) pastes
   ...
   **algorithm** name, P-C6) to B. / A가 메쉬 **Configs ▸ Invite a member**(§6)에서 B의
...
selected" — it does not auto-open a mesh. The controls live on **Configs** (§6);
alerts on **Warnings** (§7).
/ 단일 메쉬의 홈: 한눈에 보는 요약 + 로스터. Mesh 모드 기본 페이지. **현재 메쉬가
없으면**(egress=Default network) **플레인**("On the default network — no mesh selected")
— 자동으로 열지 않는다. 컨트롤은 **Configs**(§6), 경고는 **Warnings**(§7).
...
- **Warnings link / 경고 링크:** when any warnings are active, a red *"⚠ N active —
  open Warnings"* link that jumps to the Warnings tab (§7). / 경고가 있으면 빨간 *"⚠ N
  active"* 링크 → Warnings 탭(§7)으로 이동.
...
(Capture-detection / attack alerts now have their own **Warnings** tab — §7; the
mesh's controls live on **Configs** — §6.) / 탈취 감지·공격 경고는 이제 **Warnings**
탭(§7), 메쉬 컨트롤은 **Configs**(§6).

---

## 6. Mesh mode — Configs page / Mesh 모드 — Configs 페이지
...
- **Danger zone / 위험 구역:** **Report attack** → a *type-the-mesh-name* confirm →
  ...
  creator all-clears — see §7 / GUI_CRYPTO G-3); **wipe mesh** → confirm →
  ...
  전까지 — §7/GUI_CRYPTO G-3); **wipe mesh** → 확인 → `RemoveMesh`(로컬 제거).
...
## 7. Mesh mode — Warnings page / Mesh 모드 — Warnings 페이지
...
## 8. Removed from the GUI / GUI에서 제거 (❌)
...
> **Overview · Peers · Topology · Warnings · Configs** (§3, §4, §6, §7). Traffic
...
> · Configs**(§3, §4, §6, §7). Traffic(§4)는 백엔드가 생길 때만 등장.
